    ...
    try:
        ...
        if "summary" in query or "statistics" in query or "sheets" in query:
            ...
            if len(sheet_names) > 1:
                summary += f"The file contains {len(sheet_names)} sheets: {
                    ', '.join(sheet_names)}\n\n"
            ...
            if mentioned_sheet or len(sheet_names) == 1:
                ...
                if numeric_cols:
                    summary += f"Numeric Columns Statistics for sheet '{
                        sheet_to_analyze}':\n"
        ...
        # Column analysis - need to specify which sheet if multiple
            ...
            mentioned_col = next(
                (col for col in df.columns if col.lower() in query), None)

            if mentioned_col:
                col_analysis = f"Analysis of column '{
                    mentioned_col}' in sheet '{sheet_to_analyze}':\n"
                ...
                if pd.api.types.is_numeric_dtype(df[mentioned_col]):
                    ...
                    col_analysis += f"Unique values: {
                        df[mentioned_col].nunique()}\n"

                    # Show value counts for categorical data
                    value_counts = df[mentioned_col].value_counts().head(
                        10).to_string()
                    col_analysis += f"Top values:\n{value_counts}\n"

                analysis_result = col_analysis
            else:
                analysis_result = f"Column not found in sheet '{
                    sheet_to_analyze}'. Available columns: {', '.join(df.columns)}"
        ...
        # Calculate sum - need to specify which sheet if multiple
            ...
            if not numeric_cols:
                analysis_result = f"No numeric columns found in sheet '{
                    sheet_to_analyze}' to sum."
            else:
                # Try to identify which column to sum
                mentioned_col = next(
                    (col for col in numeric_cols if col.lower() in query), None)

                if mentioned_col:
                    total = df[mentioned_col].sum()
                    analysis_result = f"Sum of '{mentioned_col}' in sheet '{
                        sheet_to_analyze}': {total}"
                else:
                    # Sum all numeric columns
                    results = f"Sums of all numeric columns in sheet '{
                        sheet_to_analyze}':\n"
